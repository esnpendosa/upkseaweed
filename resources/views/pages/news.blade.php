            ...
            <article class="reveal glass rounded-[3rem] overflow-hidden border border-white/5 flex flex-col group hover:border-upkgreen/30 transition-all duration-500 shadow-2xl" 
                ...
                <div class="h-64 relative overflow-hidden">
                    @if($article->image_path)
                        <img src="{{ asset('storage/' . $article->image_path) }}" alt="{{ $article->getLocalized('title') }}" class="w-full h-full object-cover group-hover:scale-110 transition-transform duration-700">
                    @else
